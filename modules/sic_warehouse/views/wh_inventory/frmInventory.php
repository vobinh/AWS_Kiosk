<div class="col-md-12">
	<div class="portlet">
		...
		<div class="portlet-body form">
			<!-- BEGIN FORM-->
			<form id="frm-inventory" action="<?php echo url::base() ?>warehouse/saveInventory" method="post" class="form-horizontal">
				<div class="form-body">
					<div class="row">
						<div class="col-md-12">
									...
									<div class="item-add">
										...
										<div class="col-lg-7 in-group" style="overflow: hidden;">
											...
											<div class="order-25 in-group">
												<div style="float: left; width: 65%;">
													<div class="input-icon input-icon-sm right">
														...
														<input type="text" name="txt_qty[]" data-cost="<?php echo !empty($data['pro_cost_warehouse'])?$data['pro_cost_warehouse']:0; ?>" data-per="<?php echo !empty($data['pro_per_warehouse'])?$data['pro_per_warehouse']:''; ?>" value="<?php echo !empty($data['quantity'])?$data['quantity']:0; ?>" class="form-control input-sm cls_qty decimal" placeholder="Qty">
													</div>
												</div>
											...
											<div class="order-20 in-group">
												<input type="text" name="txt_date[]" value="<?php echo (!empty($data['expire_day']) && (int)strtotime($data['expire_day']) > 0)?date_format(date_create($data['expire_day']), 'm/d/Y'):''; ?>" class="form-control input-sm date-picker" placeholder="Expiration Date">
											</div>
											<div class="order-25 in-group">
												<input type="text" name="txt_price[]" value="<?php echo !empty($data['price'])?number_format($data['price'],2,'.',''):''; ?>" class="form-control input-sm decimal" placeholder="Payment">
											</div>
										</div>
										<div class="col-lg-4 in-group info-catalog">
											<?php if(!empty($data['warehouse_id'])): ?>
										...
										<div class="col-lg-1 in-group" style="text-align: right;">
											...
											<?php endif ?>
											<input type="hidden" name="txt_hd_id[]" value="<?php echo !empty($data['warehouse_id'])?$data['warehouse_id']:''; ?>">
										</div>
									</div>
								</div>
							<?php endforeach ?>
