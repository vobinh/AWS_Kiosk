	...
	var inventoryWarehouse = function(){
		...
		var initSelect2 = function(selectElementObj, frmUsing){
			...
			selectElementObj.select2({
		        ...
		        ajax: {
		            url: "<?php echo url::base()?>warehouse/getProduct",
		            dataType: 'json',
		            type: 'POST',
	        ...
	        .on('select2-selecting', function (e) {
				...
				var cost = parseFloat(e.object.pro_cost_warehouse);
				var per  = parseFloat(e.object.pro_per_warehouse);
				...
				if(e.object.pro_shelf_life_warehouse != '0'){
					wapParent.find('input[name="txt_date[]"]').datepicker('setDate', moment().add(e.object.pro_shelf_life_warehouse, 'days').format('MM/DD/YYYY'));
				}else{
		...
		var loadDataInventory = function(){
			...
			$.ajax({
				url: '<?php echo url::base() ?>warehouse/getDataInventory',
				type: 'POST',
				dataType: 'json',
				data: ajaxParams,
			})
		...
		return {
			init: function(){
				...
				if(checkCookie('twDisplay')){
					tDisplay = getCookie('twDisplay');
				}else{
					setCookie('twDisplay', tDisplay, 1);
				}
				              ...
				              cancel: {
				            	...
				            	if(result){
				            			Kiosk.blockTableUI('.portlet-body', '391px');
				                      	$.ajax({
											url: '<?php echo url::base() ?>warehouse/deleteInventory',
											type: 'POST',
											dataType: 'json',
			...
			placeOrder: function(){
				Kiosk.blockUI();
				$.ajax({
					url: '<?php echo url::base() ?>warehouse/getPlaceOrder',
					type: 'GET'
				})
				.done(function(data) {
			...
			reviewOrder: function(){
				Kiosk.blockUI();
				$.ajax({
					url: '<?php echo url::base() ?>warehouse/getReviewOrder',
					type: 'GET'
				})
				.done(function(data) {
					$('.page-quick-wap').html(data);
			...
			addInventory: function(){
				Kiosk.blockUI();
				$.ajax({
					url: '<?php echo url::base() ?>warehouse/getAddInventory',
					type: 'GET'
				})
				.done(function(data) {
					$('.page-quick-wap').html(data);
			...
			editInventory: function(id){
				Kiosk.blockUI();
				$.ajax({
					url: '<?php echo url::base() ?>warehouse/getEditInventory',
					type: 'POST',
					data:{ 'id': id },
				})
				.done(function(data) {
