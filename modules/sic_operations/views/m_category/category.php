...
<?php if($this->mPrivileges == 'NoAccess' || (is_array($this->mPrivileges) && substr($this->mPrivileges['operations_category'], 0, 1) == '0')){ ?>
	...
	<div class="col-md-12">
		<div class="portlet solid grey-cararra bordered wap-content-category">
			...
			<div class="portlet-body rgba_white" style="position: relative;">
				<div class="col-md-12" style="padding: 10px;padding: 10px;border-bottom: 1px solid #cecece;">
					<div class="row">
						<div class="in-sm-11 in-sm-push-1">
							<button type="button" disabled class="btn green btn-filter btn-show-category" data-value="all">
								Show All
							</button>
							<?php if(!empty($listCategory)): ?>
								<?php foreach ($listCategory as $key => $valCate): ?>
									<button type="button" class="btn default btn-filter btn-show-category" data-value="<?php echo !empty($valCate['category_id'])?$valCate['category_id']:'' ?>">
										<?php echo !empty($valCate['catalog_name'])?ucfirst($valCate['catalog_name']):'' ?>
									</button>
								<?php endforeach ?>
							<?php endif ?>
						</div>
				...
				<div class="clearfix"></div>
				<div class="body-data-category" style="height: 361px;position: relative;padding-top: 10px;border-bottom: 1px solid #cecece; overflow-x: hidden;overflow-y: auto;">
...
<div class="col-lg-2 col-md-3 col-sm-6 category-template" style="display: none;">
	<div class="border add-catalog item-category">
		...
		<!-- <p class="item-size-15 data-sub-name"></p> -->
