<div class="col-md-12">
	<div class="portlet">
		...
		<form id="frm-place-order" action="<?php echo url::base() ?>catalogs/savePlaceOrder" method="POST" accept-charset="utf-8">
		<div class="portlet-body form wap-place-order">
			<div class="table-responsive table-datatable filter-hidden" style="position: relative;">
				<table class="table table-striped" id="tb-place-order" width="100%" style="margin: auto auto auto 0;">
					...
					<tbody>
						<?php if(!empty($mlist_product)): ?>
							<?php foreach ($mlist_product as $key => $value): ?>
								<?php 
									$tdCostWarehouse = (!empty($value['pro_cost_store'])?'$'.$value['pro_cost_store'].' per':'').(!empty($value['pro_per_store'])?' '.$value['pro_per_store'].' '.$value['pro_unit']:'');
								 ?>
								<tr>
									<td>
									...
									<td>
										<input style="width: 85px;display: inline-block;" type="text" name="txt_unit[]" class="form-control input-sm decimal" data-cost="<?php echo !empty($value['pro_cost_store'])?$value['pro_cost_store']:'0' ?>" data-per="<?php echo !empty($value['pro_per_store'])?$value['pro_per_store']:'0' ?>" value="" placeholder="Enter"> <?php echo !empty($value['pro_unit'])?$value['pro_unit']:'Units' ?>
									</td>
									<td>
									...
									<td>
										<select name="txt_order_from[]" class="form-control input-sm cls-order-from">
											<?php if($chk_show_warhouse != false){ ?>
												<option value="warehouse">Warehouse</option>
											<?php } ?>
											<option value="independent_purchase">Independent Purchase</option>
										</select>
									</td>
								</tr>
							<?php endforeach ?>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('.cls-order-from').select2({ minimumResultsForSearch: -1 });
	});
