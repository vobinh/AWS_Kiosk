			...
			<form id="frm-catalog" action="<?php echo url::base() ?>catalogs/saveCatalog" method="post" class="form-horizontal" enctype="multipart/form-data">
				<div class="form-body">
					<div class="row">
						...
						<div <?php if(!empty($data_menu['menu']) && $data_menu['menu']['m_type'] == 2){ ?> style="display:none" <?php } ?> class="col-lg-4 col-md-4 col-sm-6">
							<div class="border add-catalog" style="height: 220px; text-align: center;">
								...
								<div class="thumbnail" style="width: 125px;position: relative; margin: auto; margin-bottom: 10px;">
									<a href="javascript:;" class="btn-remove btn btn-sm btn-circle red" style="position: absolute;top: 2px;right: 2px;margin: auto;height: 30px;width: 30px;line-height: 21px;<?php echo empty($data_menu['menu']['file_id'])?' display:none;':'' ?>"><i class="fa fa-times"></i></a>
									<img width="120px" height="110px" class="img-preAPI" src="<?php echo !empty($data_menu['menu']['file_id'])?$this->hostGetImg .'?files_id='.$data_menu['menu']['file_id']:'http://www.placehold.it/120x110/EFEFEF/AAAAAA&amp;text=no+image' ?>" alt=""/>
								</div>
								<button class="btn btn-sm green btn-upload-img" type="button">Upload</button>
								<input type="hidden" id="uploadfilehd" name="uploadfilehd" value="<?php echo !empty($data_menu['menu']['file_id'])?$data_menu['menu']['file_id']:'' ?>">
							</div>
						</div>
						...
						<div <?php if(!empty($data_menu['menu']) && $data_menu['menu']['m_type'] == 2){ ?> style="display:none" <?php } ?> class="col-lg-2 col-md-4 col-sm-6">
							<div class="border add-catalog" style="height: 220px;">
								<h4>Location</h4>
								<div class="catalog-middle in-group">
							...
							<div class="border add-catalog" style="height: 220px;">
								...
								<div class="catalog-middle" style="top: 35px;text-align: center;">
									<div style="height:120px;overflow: auto;" class="wrap-tb-option">
										<?php if(!empty($data_menu['menu_option'])): foreach ($data_menu['menu_option'] as $key => $M_Options): ?>
											<input type="hidden" name="txt_menu_parent[]" value="<?php echo $M_Options['sub_category_id']; ?>" aria-invalid="false" class="valid">
											<input type="hidden" name="txt_menu_option[]" value="<?php echo $M_Options['ref_menu_id']; ?>">
											<input type="hidden" name="txt_menu_option_price[]" value="<?php echo $M_Options['add_price']; ?>">
											<input type="hidden" name="txt_menu_option_type[]" value="<?php echo $M_Options['op']; ?>">
								...
								<div style="position: absolute;bottom: 10px;left: 10px;right: 10px;text-align: center;">
									<button onclick="StandarItem.OpenOptions()" style="position:relative" type="button" class="btn green">
										<span class="badge badge-danger total-options" style="position: absolute;top: -12px;right: -10px;background-color: #ea111b;color: #fff;font-weight: bold;font-size: 17px !important;padding: 4px 8px;height: 26px;border-radius: 50px !important;"><?php echo !empty($data_menu['Total_Option_Menu'])?$data_menu['Total_Option_Menu']:''; ?></span>
										Add Option
									</button>
								</div>
							</div>
						...
						<div class="col-lg-2 col-md-4 col-sm-6">
							<div class="border add-catalog" style="height: 220px;">
								<h4>Menu Price</h4>
								<div class="catalog-middle in-group">
									<input type="text" style="text-align: right;" class="form-control input-sm" name="txt_price" value="<?php echo !empty($data_menu['menu']['price'])?number_format($data_menu['menu']['price'],2,'.',''):'0.00' ?>" placeholder="">
								</div>
							</div>
						</div>
						<?php if(!empty($data_menu['menu']['m_type']) && ($data_menu['menu']['m_type'] == 1 || $data_menu['menu']['m_type'] == 2)){ ?>
							<div class="col-lg-2 col-md-4 col-sm-6">
								<div class="border add-catalog" style="height: 220px;">
						...
						<?php }elseif(!empty($data_menu['menu']['m_type']) && $data_menu['menu']['m_type'] == 3){ ?>
							<div class="col-lg-2 col-md-4 col-sm-6">
								<div class="border add-catalog" style="height: 220px;">
									<h4>Combo Options</h4>
									<div class="in-group" style="position: relative;">
										Select the menu items that constitute this combo set entry. Inventory connection is configured from constituent items.
									</div>
									...
									<div style="position: absolute;bottom: 10px;left: 10px;right: 10px;text-align: center;">
										...
										<span onclick="ComboSet.SelectItemMenu()" class="btn green" style="position:relative">
											<span class="badge badge-danger total-item" style="position: absolute;top: -12px;right: -10px;background-color: #ea111b;color: #fff;font-weight: bold;font-size: 17px !important;padding: 4px 8px;height: 26px;border-radius: 50px !important;"><?php echo (!empty($data_menu['ingredient']))?count($data_menu['ingredient']):'' ?></span>
											<span>Select Items</span>
										</span>
									</div>
						...
						<?php if(!empty($data_menu['menu']['m_type']) && $data_menu['menu']['m_type'] != 2){ ?>
						<div class="col-lg-2 col-md-4 col-sm-6">
							<div class="border add-catalog" style="height: 220px;">
								<h4>Barcode</h4>
								<div class="catalog-middle in-group">
								...
								<div style="position: absolute;bottom: 10px;left: 10px;right: 10px;text-align: center;">
									<span onclick="ComboSet.Generate()" class="btn green" style="position:relative">
										<span>Generate</span>
									</span>
								</div>
							</div>
				...
				<div class="form-actions right">
					<input type="hidden" name="txt_m_type" value="<?php echo ((!empty($data_menu['menu']['m_type']) && $data_menu['menu']['m_type'] == 3)?'combo_set':(!empty($data_menu['menu']['m_type']) && $data_menu['menu']['m_type'] == 1)?'standar_item':'option') ?>">
...
$(document).ready(function() {
	...
	<?php if($role == 'NoAccess' || (is_array($role) && substr($role['customers'], -1) == '0')){ ?>
		$(document).ready(function() {
			$('#frm-catalog').attr('action', 'javascript:void(0);');
		});
	<?php } ?>
});
</script>
