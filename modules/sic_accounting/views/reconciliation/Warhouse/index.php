<style>
	...
	select[name="tb-warhouse-accounting_length"]{
					...
					<div class="portlet-title">
						...
						<div class="col-lg-4 col-sm-6" style="padding-left:0">
							...
							<div class="div-btn-top div-btn-top-custom" style="margin-top: 10px;">
								<button type="button" onclick="Warhouse_Accounting.Date_Filter('today')" class="btn default btn-sm <?php if(!empty($dataFilter['typeDate']) && $dataFilter['typeDate'] == 'today'){ ?>green<?php } ?>">Today</button>
								<button type="button" onclick="Warhouse_Accounting.Date_Filter('7day')" class="btn default btn-sm <?php if(!empty($dataFilter['typeDate']) && $dataFilter['typeDate'] == '7day'){ ?>green <?php } ?>">7 days</button>
								<button type="button" onclick="Warhouse_Accounting.Date_Filter('30day')" class="btn default btn-sm <?php if(!empty($dataFilter['typeDate']) && $dataFilter['typeDate'] == '30day'){ ?>green <?php } ?>">30 days</button>
							</div>
						...
						<div class="col-lg-5 col-sm-6" style="padding-left:0">
							...
							<div class="div-btn-top div-btn-top-custom" style="margin-top: 10px;">
								<button type="button" onclick="Warhouse_Accounting.sFilter(1,'00:00|23:59')" class="btn btn-sm <?php if(!empty($dataFilter['typeFilter']) && $dataFilter['typeFilter'] == '1'){ ?>green<?php } ?>">Do not apply</button>
								<?php if(!empty($dataFilter['dataShifts'])): $slShifts = 1; ?>
									<?php foreach ($dataFilter['dataShifts'] as $shift_title => $shift): $slShifts++; ?>
										<button type="button" onclick="Warhouse_Accounting.sFilter(<?php echo $slShifts ?>,'<?php echo $shift ?>')" class="btn btn-sm <?php if(!empty($dataFilter['typeFilter']) && $dataFilter['typeFilter'] == $slShifts){ ?>green<?php } ?>"><?php echo $shift_title ?></button>
									<?php endforeach ?>
								<?php endif ?>
							</div>
							<div style="padding-bottom: 10px;overflow: hidden;">
									...
									<select id="slt_store_active" name="slt_store_active" class="form-control input-sm" style="width:100%">
										...
										<?php if(!empty($listStore)): ?>
											<?php foreach ($listStore as $LStore): ?>
												<option value="<?php echo $LStore['store_id'] ?>"><?php echo $LStore['store']; ?></option>
											<?php endforeach; ?>
										<?php endif; ?>
									</select>
								</div>
						...
						<div class="col-lg-12 col-sm-12" style="padding-left:0">
							<div class="div-btn-top" style="margin-top: 10px;">
								<button type="button" onclick="Warhouse_Accounting.whType('purchases')" class="btn btn-sm <?php if(!empty($dataFilter['typedetail']) && $dataFilter['typedetail'] == 'purchases'){ ?>green<?php } ?>">Warehouse Purchases</button>
								<button type="button" onclick="Warhouse_Accounting.whType('distribution')" class="btn btn-sm <?php if(!empty($dataFilter['typedetail']) && $dataFilter['typedetail'] == 'distribution'){ ?>green<?php } ?>">Distribution</button>
							</div>
						</div>
					</div>
					<div class="clearfix"></div>
					<div class="portlet-body" style="background-color: #ffffff;">
						<?php if(!empty($dataFilter['typedetail']) && $dataFilter['typedetail'] == 'purchases'): ?>
							<div class="row">
								<div class="col-lg-7">
									<p style="line-height: 28px;font-weight: 600; margin:5px;">
										Items added to warehouse stock (purchase)
									</p>
								</div>
								<?php if($role == 'FullAccess' || (is_array($role) && substr($role['acc_recon'], -1) == '1')): ?>
								<div class="col-lg-5 cls-right">
									<button style="margin:5px;" type="button" data-val="warehouse" class="btn btn-sm green btn-adjustment">Enter Adjustment</button>
								</div>
								<?php endif ?>
							</div>
							<div class="table-datatable filter-hidden" style="position: relative;">
								<table class="table table-striped table-bordered table-nobordered table-hover" id="tb-warhouse-accounting" width="100%" style="margin: auto auto auto 0;">
									<thead>
										<tr>
											...
											<th class="cls-center">Payment Received</th>
										</tr>
									</thead>
									<tbody>
										<?php $total_last = 0; ?>
										...
										<?php if(!empty($dataOrder)): ?>
											<?php foreach ($dataOrder as $key => $value): ?>
												...
												<tr>
													...
													<td class="cls-center">
														<?php echo !empty($value['w_unit'])?$value['w_unit']:'' ?><?php echo !empty($value['pro_unit'])?' '.$value['pro_unit']:'' ?>
													</td>
													<td class="cls-center">
														$<?php echo !empty($value['w_price'])?number_format($value['w_price'],2,'.',''):'0.00' ?>
													</td>
												</tr>
											<?php endforeach; ?>
										<?php endif; ?>
									...
									<tfoot>
							            <tr>
							                <th colspan="6" class="cls-right">Total Payment Received:</th>
							                <th class="cls-center">
							                	$<?php echo !empty($total_last)?number_format($total_last,2,'.',''):'0.00' ?>
							                </th>
							            </tr>
							        </tfoot>
								</table>
						...
						<?php else: ?>
							<div class="row">
								<div class="col-lg-7">
									<p style="line-height: 28px;font-weight: 600; margin:5px;">
										Warehouse items sent to stores (distribution)
									</p>
								</div>
								<?php if($role == 'FullAccess' || (is_array($role) && substr($role['acc_recon'], -1) == '1')): ?>
								<div class="col-lg-5 cls-right">
									<button style="margin:5px;" type="button" data-val="distribution" class="btn btn-sm green btn-adjustment">Enter Adjustment</button>
								</div>
								<?php endif ?>
							</div>
							<div class="table-datatable filter-hidden" style="position: relative;">
								<table class="table table-striped table-bordered table-nobordered table-hover" id="tb-warhouse-accounting" width="100%" style="margin: auto auto auto 0;">
									<thead>
										<tr>
											...
											<th class="cls-center">Item</th>
											<th class="cls-center">SKU#</th>
											<th class="cls-center">Recipient Store</th>
											<th class="cls-center">Quantity</th>
											<th class="cls-center">Payment Received</th>
										</tr>
									</thead>
									<tbody>
										...
										<?php if(!empty($dataOrder)): ?>
											<?php foreach ($dataOrder as $key => $value): ?>
												...
												<?php 
													$price_detail = !empty($value['store_price'])?$value['store_price']:0;
													$total_last += $price_detail;
												?>
												<tr>
													<td class="cls-center">
														<?php echo !empty($value['date_mark_warehouse_complete'])?date('H:i:s', strtotime($value['date_mark_warehouse_complete'])):'' ?>
													</td>
													<td class="cls-center">
														<?php echo !empty($value['date_mark_warehouse_complete'])?date('m/d/Y', strtotime($value['date_mark_warehouse_complete'])):'' ?>
													</td>
													<td class="cls-center">
														<?php echo !empty($value['sub_category_name'])?$value['sub_category_name']:'' ?>
													</td>
													...
													<td class="cls-center">
														<?php echo !empty($value['store'])?$value['store']:'' ?>
													</td>
													<td class="cls-center">
														<?php echo !empty($value['store_unit'])?$value['store_unit']:'' ?><?php echo !empty($value['pro_unit'])?' '.$value['pro_unit']:'' ?>
													</td>
													<td class="cls-center">
														$<?php echo !empty($price_detail)?number_format($price_detail,2,'.',''):'0.00' ?>
													</td>
												</tr>
											<?php endforeach; ?>
										<?php endif; ?>
									...
									<tfoot>
							            <tr>
							                <th colspan="7" class="cls-right">Total Payment Received:</th>
							                <th class="cls-center">
						                	$<?php echo !empty($total_last)?number_format($total_last,2,'.',''):'0.00' ?>
						                </th>
						            </tr>
