...
function close_form_crop_company()
   {
    $( "#wrap_crop_company_register" ).dialog( "close" );
  }
</script>
<div  class="container">
...
<!-- <div class="action_company"> -->
  ...
  <button class="btn" id="btnCrop_company" value="Crop" style="float: right;  width: 150px;margin-right: 5px;display:none">Crop</button>
<!-- </div> -->

<button class="btn" id="btnCrop_company_cancel" value="Cancel" style="float: right;  width: 150px;margin-right: 5px;">Cancel</button>

<script type="text/javascript">
  $(function(){
    $('#btnCrop_company_cancel').click(function(){
     ...
     var options =
        ...
        document.querySelector('#file_company').addEventListener('change', function(){
          ...
          if ($.inArray($(this).val().split('.').pop().toLowerCase(), fileExtension) == -1) {
            ...
            $(this).files = [];
            $('#btnCrop_company').show();
            $('#btnCrop_company_cancel').hide();
        })
        document.querySelector('#btnCrop_company').addEventListener('click', function(){
          $('#loading_img').show();
            var img = cropper.getDataURL();
            //$('#image_company_menu').html('<img src="'+img+'">')
            document.querySelector('#image_company_menu').innerHTML='';
            document.querySelector('#image_company_menu').innerHTML += '<img src="'+img+'">';
            close_form_crop_company();
            $.ajax({
                type: 'POST',
                url: '<?php echo url::base() ?>info/save_img_company',
                data: {image: img},
                success: function (resp) { 
                   document.querySelector('#image_company_menu').innerHTML += '<input type="hidden" value="'+resp+'" name="image_company">';
                   $('#loading_img').hide();
          ...
          $.fn.customFile = function() {
            return this.each(function() {
              var $file = $(this).addClass('custom-file-upload-hidden_company'), // the original file input
                  ...
                  $button = $('<button type="button" class="file-upload-button_company btn">Browse…</button>'),
                  // Hack for IE
                  $label = $('<label class="file-upload-button_company" for="'+ $file[0].id +'">Browse…</label>');
