...
<form style="padding-top: 2%;padding-bottom: 2%;text-align:center" class="form_card" method="POST"   action="<?php echo url::base() ?>account/save_card_signle" id="form_nap_card">
    <table class="table_form_card" width="100%" >
      ...
      <tr>
        <td>
         <input onkeypress="return isNumberKey(event)" maxlength="2" name="month" placeholder="Month" style="width:97px;float: none;" data-stripe="exp-month" type="text">
          /
          <input onkeypress="return isNumberKey(event)" maxlength="4" name="year" placeholder="Year" data-stripe="exp-year" style="width:91px;float: none;" type="text">
        </td>
      </tr>
      <tr>
        <td>
      ...
      <tr>
        <td>
          <input type="text" style='fl  oat: none;' data-stripe="address_city" name="city" placeholder="City">
        </td>
      </tr>
      <tr>
        <td>
      ...
      <tr>
        <td>
          <select style="width:89%;float: none"  name="place" id="">
  ...
  <div style="margin-left: 16px;">
      ...
      <button style="width: 110px;float: right;margin-right: 16px" class="btn cancel_card" type="button">Cancel</button>
  </div>
</form>
<?php 
    ...
    $.growl.error({ message: response.error.message });
      ...
      $(function() {
        $.ajax({
          type: "POST",
          dataType:"JSON",
          url: "<?php echo url::base() ?>ajax_progress/save_card_signle",
          data: {data:data,data1:data1},
          success: function(resp){
            if(resp['message'] == true){
                $("#wrapcard").dialog( "close" );
                $(".form_credit_cc").dialog( "close" );
                $('#loading_img').hide();
                window.location.href='<?php echo url::base() ?>create/fregister_single';
            }else{
                $('#cofirm').attr('disabled',false);
                $('#loading_img').hide();
